...
def text_to_speech(text, output_path):
    """Converts text to speech using gTTS and returns an MP3 path.

    MoviePy can read MP3 directly via its bundled ffmpeg, avoiding external ffprobe.
    """
...
def generate_visuals(output_dir, video_type, slide_content=None, thumbnail_title=None, slide_number=0, total_slides=0):
    """Generates a slide/thumbnail. Falls back to a simple solid image if anything fails."""
    try:
        output_dir.mkdir(exist_ok=True, parents=True)
        is_thumbnail = thumbnail_title is not None
        width, height = (1920, 1080) if video_type == 'long' else (1080, 1920)
        title = thumbnail_title if is_thumbnail else (slide_content.get("title", "") if slide_content else "")

        bg_image = get_pexels_image(title, video_type)
        if not bg_image:
            bg_image = Image.new('RGBA', (width, height), color=(12, 17, 29))
        bg_image = bg_image.resize((width, height)).filter(ImageFilter.GaussianBlur(5))
        final_bg = Image.alpha_composite(bg_image, Image.new('RGBA', bg_image.size, (0,0,0,150))).convert('RGB')
        ...
        if not is_thumbnail:
            ...
            for w in words:
                ...
                if bbox[2]-bbox[0] < width*0.9:
                    ...
                    title_lines.append(current); current=w
            title_lines.append(current)
            bboxA=draw.textbbox((0,0),"A", font=title_font)
            line_h=(bboxA[3]-bboxA[1])+10
            total_h=len(title_lines)*line_h
            y=(header_height-total_h)/2
        ...
        if not is_thumbnail:
            content = (slide_content.get("content", "") if slide_content else "")
            words=content.split()
            lines=[]; current=""
            for w in words:
                test=f"{current} {w}".strip()
            ...
            bboxA=draw.textbbox((0,0),"A", font=content_font)
            line_h=(bboxA[3]-bboxA[1])+15
            is_special=len(content.split())<10
            total_h=len(lines)*line_h
            y = (height-total_h)/2 if is_special else header_height+100
            for line in lines:
                bbox=draw.textbbox((0,0), line, font=content_font)
                x=(width-(bbox[2]-bbox[0]))/2
                draw.text((x,y), line, font=content_font, fill=(230,230,230))
                y+=line_h
            footer_h=int(height*0.06)
            draw.rectangle([0,height-footer_h,width,height], fill=(25,40,65,200))
            draw.text((40,height-footer_h+12), f"AI for Developers by {YOUR_NAME}", font=footer_font, fill=(180,180,180))
            if total_slides>0:
                slide_txt=f"Slide {slide_number} of {total_slides}"
                bbox=draw.textbbox((0,0), slide_txt, font=footer_font)
                draw.text((width-bbox[2]-40, height-footer_h+12), slide_txt, font=footer_font, fill=(180,180,180))
        ...
        print(f"❌ Slide generation failed, using fallback simple image: {e}")
        output_dir.mkdir(exist_ok=True, parents=True)
        width, height = (1920,1080)
        img = Image.new('RGB',(width,height),(20,30,45))
        draw = ImageDraw.Draw(img)
        msg = (thumbnail_title or (slide_content.get('title') if slide_content else 'NEWS'))[:40]
        draw.text((40,40), msg, fill=(255,255,255))
        path = output_dir / "fallback_slide.png"
        img.save(path)
        return str(path)
